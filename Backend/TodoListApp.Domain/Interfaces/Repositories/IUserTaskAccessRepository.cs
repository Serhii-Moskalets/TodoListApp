...
public interface IUserTaskAccessRepository : IRepository<UserTaskAccessEntity>
{
    ...
    Task<IReadOnlyCollection<TaskEntity>> GetSharedTasksForUserAsync(Guid userId, CancellationToken cancellationToken = default);
    ...
    /// <summary>
    /// Checks whether a user has access to any task in a specific task list.
    /// </summary>
    /// <param name="userId">The user identifier.</param>
    /// <param name="todoListId">The task list identifier.</param>
    /// <param name="cancellationToken">A <see cref="CancellationToken"/> to observe while waiting for the task to complete.</param>
    /// <returns><c>true</c> if the user has access to any task in the list; otherwise, <c>false</c>.</returns>
